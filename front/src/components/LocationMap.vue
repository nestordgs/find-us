<template>
  <div>
    <v-layout row wrap>
      <v-flex xs12 sm5>
        <v-text-field label="Longitud" name="longitud" id="longitud" v-model="data.longitud" :rules="lngRules"
                      v-on:keyup="longitud"></v-text-field>
      </v-flex>
      <v-flex xs12 sm5 offset-sm1>
        <v-text-field label="Latitud" name="latitud" id="latitud" v-model="data.latitud" required :rules="latRules"
                      v-on:keyup="latitud"></v-text-field>
      </v-flex>
      <v-card-text style="position: relative">
        <v-btn v-tooltip:left="{ html: 'Ver Ubicacion en el Mapa' }" absolute dark outline fab top right
               class="indigo btn--small" @click="showMap(data.longitud,data.latitud)">
